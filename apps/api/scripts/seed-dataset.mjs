...
async function main() {
  ...
  for (let i = 1; i <= 200; i++) {
    ...
    sql.push(
      insertOnConflictId(
        'comments',
        ['id', 'content_id', 'author', 'body', 'status', 'created_at', 'approved_at'],
        [commentId, video.id, `User${pad3(1 + Math.floor(rand() * 30))}`, pick(rand, commentBodies), status, createdAt, approvedAt]
      )
    )
  }
  ...
  // cms_rankings (simple synthetic)
  sql.push('-- cms_rankings')
  const rankingTypes = ['plays_today', 'coins_today', 'favorites']
  rankingTypes.forEach((type) => {
    const top = pickN(rand, videos, 10)
    top.forEach((v, idx) => {
      const rank = idx + 1
      const value = 1000 - idx * 37 + Math.floor(rand() * 20)
      sql.push(
        [
          'INSERT INTO cms_rankings (type, as_of, rank, entity_id, label, value) VALUES',
          `  (${q(type)}, ${q(now.slice(0, 10))}, ${rank}, ${q(v.id)}, ${q(v.title)}, ${value})`,
