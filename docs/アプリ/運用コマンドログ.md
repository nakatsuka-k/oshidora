...
## 重要：Cloudflare プロジェクト名（誤字防止）

※ **`oshidora` ではなく `oshidra`**。ここに書いた名前をコピペしてください。

| 種類 | プロジェクト名 | URL |
|------|--------------|-----|
| Pages | `oshidra-web` | https://oshidra-web.pages.dev |
| Workers API | `oshidra-api` | https://oshidra-api.kousuke-c62.workers.dev |
| Workers Uploader | `oshidra-uploader` | https://assets-uploader.oshidra.com |

### Pages デプロイ（正）

- `npm run build:web`
- `wrangler pages deploy apps/mobile/dist --project-name oshidra-web --branch main`
...
※ **Cloudflareプロジェクト名は以下の通りです。誤字に注意（`oshidora` ではなく `oshidra`）**
...
- 許可IPはデフォルトの allowlist（リポジトリ側で管理）で制限され、環境変数 `OSHIDORA_ALLOWED_IPS`（カンマ区切り）で上書き可能
